 ...
 * @author Z
 */
@WebServlet(name = "AllKayaks", urlPatterns = {"/AllKayaks"})
public class AllKayaks extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isAuthenticated){
            ...
            session.setAttribute("kayaks", allKayaks);
            session.setAttribute("user", user);
            session.setAttribute("username", username);

            response.sendRedirect("showallkayaks.jsp");
        }else {
            response.getWriter().println("Du er ikke logget ind");
        }
